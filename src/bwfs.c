...
#endif
// Definicion de mascaras importantes
#ifndef S_IFDIR // Indica que el archivo es un directorio
#define S_IFDIR 0040000
#endif

#ifndef S_IFREG //Indica que el arhcivo es un archivo regular
#define S_IFREG 0100000
#endif
#ifndef S_IFMT  // Indica que es un archivo
#define S_IFMT 0100000
#endif
 ...
 * 
 */
// Quitar las barras consecutivas redundantes (//) era un bug que nos generaba al pasar el mount...
static void compact_slashes(const char *src, char *dst, size_t dstsz) {
    ...
    for (size_t i = 0; src[i] && j + 1 < dstsz; ++i) {
        dst[j++] = src[i]; 
        if (src[i] == '/') {
            while (src[i + 1] == '/') i++; // salto de las barras que son duplicadas
        }
    }
    dst[j] = '\0';
}
...
// Verifica que exista el directorio de storage, sino lo crea
...
static int load_metadata(void) {
    bwfs_disk_t disk; // Lectura dde datos desde el archivo metadata
    char meta_path[PATH_MAX]; // Ruta absoluta al archivo de metada
    ...
    //////      Inicializa los inodos en la memoria    ////
    for (int i = 0; i < MAX_FILES; ++i) {
        ...
        if (disk.inodes[i].used) {
            ...
            bwfs.inodes[i].name[MAX_NAME_LEN] = '\0'; // Asegura un terminador nulo
    ...
    // Retorno a si se carga correctamente el metadata
    return 0;
}
...
static int save_metadata(void) {
    ...
    for (int i = 0; i < MAX_FILES; ++i) {
        disk.inodes[i].used = bwfs.inodes[i].used;  // Se marca si el inodo está marcado
    ...
    fclose(fp);
    // Retorno 0 si fue correcta la escritura, sino -1 cuando falla
    return (w == 1) ? 0 : -1;
}
...
static void *bwfs_init(struct fuse_conn_info *conn, struct fuse_config *cfg)
{
    ...
    if (load_metadata() != 0) {
        fprintf(stderr,
            "[bwfs_init] ERROR: No existe metadata válida.\n"
            "Debe ejecutar primero: mkfs.bwfs -c config.ini\n"
        );
        // Al fallar init(), FUSE aborta el montaje
        exit(1);
    }

    // 2. Metadata cargada → validar storage
    if (ensure_storage_dir_exists() != 0) {
        fprintf(stderr, "[bwfs_init] ERROR: storage_path inválido.\n");
        exit(1);
    }

    fprintf(stderr, "[bwfs_init] OK: metadata cargada.\n");
    return NULL;
}
...
static int find_inode_by_name(const char *path) {
    ...
    if (!fname || !fname[0]) return -1; // path vacio o no valido
...
static int allocate_inode(const char *path, mode_t mode) {
    ...
    if (!fname || !fname[0]) return -EINVAL;   // Nombre invalido
    if (strlen(fname) > MAX_NAME_LEN) return -ENAMETOOLONG; // Nombre muy extenso
    if (find_inode_by_name(path) >= 0) return -EEXIST; // Ya existe un archivo con ese nombre
    // Valida que storage path esté definido y exista
    if (bwfs.storage_path[0] == '\0') return -EIO; //Path no configurado
    if (ensure_storage_dir_exists() != 0) return -EIO; // Error creando el directorio
        ...
        if (!bwfs.inodes[i].used) { // En caso que el nodo esté libre, se incializa
            ...
            if (fd < 0) {
                if (errno == EEXIST) {
                    ...
                    if (fd < 0) { // En caso que falle el truncamiento
            ...
            //Hace un retorno al índice del inodo asignado
            return i;
        }
    }
    // Return para cuando no se encuentran inodos libres
    return -ENOSPC;
}
...
static int bwfs_unlink(const char *path) {
    ...
    const char *fname = basename_from_path(path);
    if (!fname) return -ENOENT; // Ruta invalida
...
static int bwfs_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
    ...
    inode_t *ino = &bwfs.inodes[idx];
    if ((size_t)offset >= ino->size) return 0; // Archivo no existe
 ...
 */
 static int bwfs_utimens(const char *path, const struct timespec tv[2], struct fuse_file_info *fi) {
    ...
    return -ENOENT; // En caso que se termine el ciclo sin encontrar el archivo, no existe
}
/**
