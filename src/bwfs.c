...
/* Track recently created files to avoid remote read while user is creating them */
#define MAX_RECENT_FILES 64
static char recent_files[MAX_RECENT_FILES][PATH_MAX];
static int  recent_files_count = 0;

static void mark_file_recently_created( const char *path ) {
  if ( !path || recent_files_count >= MAX_RECENT_FILES )
    return;
  ...
  // Add to list
  strncpy( recent_files[recent_files_count], path, PATH_MAX - 1 );
  recent_files[recent_files_count][PATH_MAX - 1] = '\0';
  recent_files_count++;
}

static int is_file_recently_created( const char *path ) {
  if ( !path )
    return 0;
...
static void clear_recent_file( const char *path ) {
  if ( !path )
    return;
  ...
  for ( int i = 0; i < recent_files_count; i++ ) {
    if ( strcmp( recent_files[i], path ) == 0 ) {
      ...
      for ( int j = i; j < recent_files_count - 1; j++ ) {
...
static int has_extension( const char *path ) {
  ...
  // Check that there's at least 3 characters after the dot
  // (to avoid matching partial extensions like .t, .tx while typing .txt)
  // and the dot is not at the start of the filename
  // This covers most common extensions (.txt, .dat, .log, .pdf, etc.)
  return ( last_dot > fname && last_dot[1] != '\0' && last_dot[2] != '\0' &&
           last_dot[3] != '\0' );
}
...
static int bwfs_getattr( const char            *path,
  ...
  if ( idx < 0 ) {
    // Skip remote read if file was recently created (user is actively creating it)
    if ( is_file_recently_created( path ) ) {
      return -ENOENT;
    }

    // Only try reading remote if the file path has an extension
    if ( has_extension( path ) ) {
      ...
      // File doesn't exist and has no extension, return error
      return -ENOENT;
    }
  }
...
bwfs_create( const char *path, mode_t mode, struct fuse_file_info *fi ) {
  ...
  // Mark file as recently created to skip remote read
  mark_file_recently_created( path );
  return 0;
}
...
static int bwfs_write( const char            *path,
  ...
  // Clear from recent files list once user starts writing (file is confirmed)
  clear_recent_file( path );
